...
public class FunctionRefractorFrenchRevenueTaxCalculator {
    ...
    public static void main(String[] args) {
        ...
        printSalaryHeader(netTaxableSalary_5);
        // Conditions en cascade , avec soustraction pour chaque tranche.
        //  > Tranche 1
        if (netTaxableSalary_5 < TAX_THRESHOLDS_1){
            System.out.println("Aucune impôt à payer");
        }
        else {

            double bracket4Tax = calculateTaxForBracket(netTaxableSalary_5, TAX_THRESHOLDS_4, TAX_RATE_4, bracket4Label);
                 if (bracket4Tax > 0){
                     totalTax += bracket4Tax;
                     netTaxableSalary_5 = TAX_THRESHOLDS_4;
                 }

            double bracket3Tax = calculateTaxForBracket(netTaxableSalary_5, TAX_THRESHOLDS_3, TAX_RATE_3, bracket3Label);
                if (bracket3Tax > 0){
                    totalTax += bracket3Tax;
                    netTaxableSalary_5 = TAX_THRESHOLDS_3;
                }

            double bracket2Tax = calculateTaxForBracket(netTaxableSalary_5, TAX_THRESHOLDS_2, TAX_RATE_2, bracket2Label);
                if (bracket2Tax > 0) {
                    totalTax += bracket2Tax;
                    netTaxableSalary_5 = TAX_THRESHOLDS_2;
                }

            double bracket1Tax = calculateTaxForBracket(netTaxableSalary_5, TAX_THRESHOLDS_1, TAX_RATE_1, bracket1Label);
                if (bracket1Tax > 0){
                    totalTax += bracket1Tax;
                    netTaxableSalary_5 = TAX_THRESHOLDS_1;
                }
        }
        // Total
    ...
    /**
     * Displays the formatted header and taxable salary amount for the user.
     * @param netTaxableSalary the user's total net taxable income.
     */
    public static void printSalaryHeader(double netTaxableSalary) {
        System.out.println("------ Salaire Net Imposable ------");
        System.out.println("Votre salaire : " + netTaxableSalary+ " €");
