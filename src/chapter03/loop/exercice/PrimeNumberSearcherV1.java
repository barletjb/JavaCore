...
public class PrimeNumberSearcherV1 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= n; i++) {
            //chapter01.variable that keeps track of the current number as prime or not.
            boolean isPrime = true;
            for (int j = i - 1; j >= 2; j--) {

                //If the number can divide then it cannot be prime.
                //So terminate the chapter03.loop by marking it.
                if (i % j == 0) {
                    isPrime = false;
                    break;
                }
            ...
            //If the chapter01.variable is not modified by the inner chapter03.loop then it is prime.
            if (isPrime)
                System.out.print(i + " ");
        }
    }
